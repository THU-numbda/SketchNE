...
void gbbs_util<Graph,FP>::gbbs_sparse_mm(Graph& GA, mat<FP> *X, mat<FP> *Y,mat<FP> *degree_alpha){
  ...
  */
  ...
  parallel_for(0, GA.n, [&] (MKL_INT i) {
    FP factor = degree_alpha->d[i];
    auto map_f = [&] (const uintE& u, const uintE& v, const W& wgh) {
      if (u!=v){
        FP factor_new = factor * degree_alpha->d[v];
        mklhelper<FP>::cblas_axpy(d,factor_new,X->d+v*d,1,Y->d+u*d,1);
      }
    };
...
mat<T> *util<T>::matrix_new(MKL_INT nrows,MKL_INT ncols){
  ...
  //M->d = new T[size_mat]();
  //M->d = (T*) mkl_calloc(size_mat,sizeof(T),ALIGN);//calloc may very slow for a large size_mat
...
template <typename T>
void util<T>::linear_solve_Uxb(mat<T> *A, mat<T> *b)
{
  MKL_INT k = std::min(A->nrows,A->ncols);
  lapack_int *ipiv = new lapack_int[k];
  lapack_int info = mklhelper<T>::LAPACKE_getrf(LAPACK_ROW_MAJOR,A->nrows,A->ncols,A->d,A->ncols,ipiv);
  if (info != 0)
    std::cout<<"some Error happen in the getrf,info:"<<info<<std::endl;
  
  info = mklhelper<T>::LAPACKE_getrs(LAPACK_ROW_MAJOR,'N',b->nrows,b->ncols,A->d,b->nrows,ipiv,b->d,b->ncols);
  if (info != 0)
    std::cout<<"some Error happen in the getrs,info:"<<info<<std::endl;
  delete ipiv;
  // lapack_int info = mklhelper<T>::LAPACKE_trtrs(LAPACK_ROW_MAJOR, 'U', 'N', 'N', //
  //                             b->nrows,
  //                             b->ncols,
  //                             A->d,
  //                             b->nrows, //ncols
  ...
  //                             b->ncols);
  // if (info != 0)
  //     printf("LAPACK_strtrs return value: %d\n", info);
}
...
void util<T>::eigSVD(mat<T> *A, mat<T> *U, mat<T> *S, mat<T> *V)
{
  ...
  #pragma omp for
      ...
      {
          // if (S->d[i]<0){
          //   std::cout<<"Before there is numerical problem when computing eigSVD, recommend to use QR(SVD)"<<std::endl;
          //   exit(1);
          // }
          S->d[i] = sqrt(S->d[i]);
          if (S->d[i]==0){
